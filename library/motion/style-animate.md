...
- 🎬 **Video** — Seedance 2.5 image-to-video (resolve live; e.g. `bytedance/seedance-2.5/image-to-video`); `bytedance/seedance-2.0-fast/image-to-video` is cheapest for drafts
...
3. **Draft the motion** — describe the style + the movement → maps to `atlas_generate_video` with **Seedance 2.5 i2v** (`bytedance/seedance-2.0-fast/image-to-video` first).
